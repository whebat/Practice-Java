...
1. Write a method called toMilesPerHour that has 1 parameter of type double
   ...
   Otherwise, if it is positive, calculate the value of miles per hour, round
   it and return it. For conversion and rounding, check the notes in the text
   below.
...
2. Write another method called printConversion with 1 parameter of type double
   ...
   If the parameter kilometersPerHour is < 0
   ...
   Use method Math.round to round the number of calculated mi/h (double).
   The method round returns long.

TIP:  In the method printConversion,
      call the method toMilesPerHour instead of duplicating the code.
